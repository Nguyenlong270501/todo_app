...
class _TaskFormSectionState extends State<TaskFormSection> {
  ...
  void initState() {
    ...
    if (widget.initialTask != null) {
      // For existing tasks, use the stored time
      final storedTime = widget.initialTask?.time;
      if (storedTime != null) {
        final timeFormat = DateFormat('hh:mm a');
        final time = timeFormat.parse(storedTime);
        _date = DateTime(
      ...
      // For new tasks, use current time
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          // Title Section
          AppSizes.gapH12,
          Text('Title', style: AppTypography.medium14()),
          AppSizes.gapH12,
          AppTextfield(
            hint: 'Enter Title',
            validator: (value) {
          ...
          // Note Section
          AppSizes.gapH24,
          Text('Note', style: AppTypography.medium14()),
          AppSizes.gapH12,
          AppTextfield(
            hint: 'Enter Note',
            validator: (value) {
          ...
          // Date Section
          AppSizes.gapH24,
          Text('Date', style: AppTypography.medium14()),
          AppSizes.gapH12,
          Row(
            children: [
              Expanded(
  ...
  void submitForm(String uid) {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (widget.initialTask != null) {
        context.read<TaskCubit>().updateTask(task, uid);
        ...
        context.read<TaskCubit>().addTask(task, uid);
        LocalNotificationService().scheduleNotification(
          "Task Reminder",
          "This is a reminder for your task: ${task.title}",
          _date,
